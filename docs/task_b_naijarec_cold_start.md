# Task B: naijarec_cold-start for Nigerian-Focused Restaurant Recommendation

## Fit

`naijarec_cold-start` adopts the Kalm4Rec method as a good backbone for Task B because the challenge rewards contextual recommendation, cold-start behavior, and reasoning before ranking. The source pipeline already does three useful things:
...
`tripAdvisor` is part of the upstream reference examples and is not part of this experiment or recommendation domain.
...
1. **Candidate retrieval**
   Run MPG keyword retrieval over your Yelp restaurant subset. This gives a candidate pool from behavior signals instead of asking the LLM to search over 8,958 restaurants.
...
After the notebook writes `user_review_history.json` and `restaurant_detail.csv`, convert them into `naijarec_cold-start` format with temporal holdout labels:
...
Then the normal model stages can run with `--city naija_yelp`.
...
2. **Source-paper user-disjoint protocol**
...
Run source-paper-style retrieval with:
