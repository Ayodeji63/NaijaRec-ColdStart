# Chapter 3 and 4 README: naijarec_cold-start Model Experiments

This README summarizes the methodology and experimental findings for `naijarec_cold-start`, which adapts the Kalm4Rec method. It is written to support Chapter 3 (methodology) and Chapter 4 (results and discussion).
...
TripAdvisor is not part of this experiment. It exists only in the upstream reference implementation as a legacy example.
...
| Source-paper protocol | `naija_yelp_paper.csv`, `naija_yelp_paper_splits.json` | User-disjoint cold-start protocol matching the Kalm4Rec paper style |
...
The retrieval stage uses MPG, the graph retrieval method adopted from the Kalm4Rec reference implementation:
...
### Source-Paper User-Disjoint MPG Retrieval
...
```text
naijarec_cold-start keyword extraction + MPG retrieval with quantity=20 + hybrid reranking with alpha=0.3
```

### Figures for Chapter 4

Generate the result figures from the recorded metric tables with:

```bash
MPLCONFIGDIR=/tmp/naijarec_mpl python taskB/plot_results.py --out_dir docs/figures
```

The NDCG summary is a presentation overview across evaluated settings; the Yelp and Amazon protocols are not assumed to have equal difficulty.

![NDCG at 20 summary across evaluated settings](figures/ndcg20_summary.png)

![Yelp cold-start method comparison](figures/yelp_model_comparison.png)

![Amazon Grocery cross-domain comparison](figures/amazon_cross_domain_comparison.png)

![Yelp keyword quantity ablation](figures/yelp_quantity_ablation.png)

![Dense Amazon hybrid metrics by cutoff](figures/dense_amazon_hybrid_by_cutoff.png)

### Recommended Chapter 4 Claim

The `naijarec_cold-start` model achieved:
